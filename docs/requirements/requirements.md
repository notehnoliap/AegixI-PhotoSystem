...
| FR-2.3 | 系统应将缩略图和元数据发送给AI模型进行分析 | 1. 系统应将缩略图和允许的元数据发送给AI模型<br>2. 系统应使用安全的传输方式，确保数据安全<br>3. 如果本地AI模型无法处理，系统应使用远程AI模型<br>4. 系统应设置超时机制，避免长时间等待<br>5. 系统应记录AI分析请求和响应日志 | 中（第二阶段） |
...
| CR-1.3 | 系统应支持配置AI模型参数 | 1. 管理员应能选择使用本地或远程AI模型<br>2. 管理员应能配置本地AI模型的资源限制<br>3. 管理员应能配置远程AI模型的API端点<br>4. 管理员应能配置AI处理的优先级规则<br>5. 管理员应能启用或禁用特定AI功能 | 中（第二阶段） |
